...
Сервис обслуживает **все** подписки в статусе `ACTIVE`, **независимо от типа движка** — это общий
upstream, совпадения нужны всем delivery-движкам (свой тип движка каждый отбирает сам). Конфигурация
подписок читается **исключительно из Redis** (Postgres не используется); доменная модель — из
DataDictionary.
...
Подписка обслуживается при `status = ACTIVE` (любой тип движка). Если её фильтр не
