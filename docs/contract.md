...
> **Первая версия объекта** (`before: null` или ключ `before` отсутствует, а `after` присутствует)
> обрабатывается как `OBJECT` по телу `after`: предыдущей ревизии нет, объект может только *войти* в
> выборку, поэтому и downstream-эффект совпадает с обычным `OBJECT` (доставляется, если совпал).
...
- присутствует только `before` без `after` (удаление — не поддерживаемая форма изменения);
  обратный случай (`before: null` + `after`) — это первая версия, она валидна и обрабатывается как `OBJECT`;
