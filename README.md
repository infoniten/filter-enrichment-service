...
5. **Конфигурация подписок — только из Redis.** Postgres не используется. Обслуживаются **все**
   подписки со `status = ACTIVE`, **независимо от типа движка** — сервис общий upstream, совпадения
   нужны всем delivery-движкам, а свой тип движка каждый из них отбирает уже сам.
